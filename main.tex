...
\begin{frame}{Interpolation der Daten}
  ...
  \begin{itemize}
    ...
    \begin{align*}
      t_n &= t_{\mathrm{A}} + \sum_{i=1}^{n} \frac{t_{\mathrm{B}}-t_{\mathrm{A}}}{n} \cdot i \\
      T_n &= T_{\mathrm{A}} + \sum_{i=1}^{n} \frac{T_{\mathrm{B}}-T_{\mathrm{A}}}{n} \cdot i \\
    \end{align*}
  \end{itemize}
\end{frame}
...
\begin{frame}{Abweichungen}
  \begin{itemize}
    ...
    \medskip
    \item Veränderung der Temperatur zwischen Messungen nicht berücksichtigt
    \medskip
    \item Messwerte unterliegen Unsicherheiten
  \end{itemize}
\end{frame}
...
\begin{frame}{Interface Beispiel: Dotierungskonzentration}
  \begin{figure}
    ...
    \caption{Beispiel Plot von $N_{\mathrm{eff}}$ mit $t = \SI{10e5}{\minute}$, $\Phi_{\mathrm{eq}}= \SI{5e-15}{\per\centi\meter\squared}$}
  \end{figure}
\end{frame}
...
\begin{frame}{Fazit}
  \begin{itemize}
    ...
    \medskip
    \item Parameter, optional angezeigtes Verhalten bei konstanter Temperatur und Anzahl an Interpolationsintervallen in den
